...
import { Clock, MapPin, Users, Calendar as CalendarIcon, Trophy, Target } from "lucide-react";
...
const matchData: Match[] = [
	{
		...
		referee: "John Smith",
		status: "scheduled",
		description: "Important league match against City Academy. This is a crucial game for maintaining our position at the top of the table.",
		objectives: [
	...
	},
	{
		...
		status: "scheduled",
		description: "Cup quarter-final away match. A challenging fixture against a strong United Youth side.",
		objectives: [
	...
	},
	{
		...
		referee: "Mike Wilson",
		status: "scheduled",
		description: "Friendly match to give younger players match experience and test new tactical approaches.",
		objectives: [
	...
	},
	{
		...
		referee: "David Brown",
		status: "scheduled",
		description: "Evening fixture under lights against Elite Academy. A test of our squad depth and tactical flexibility.",
		objectives: [
	...
	},
	{
		...
		awayScore: 1,
		status: "completed",
		description: "Excellent performance against Coastal FC with dominant display throughout the match.",
		objectives: [
			...
			return (
				<div className="flex justify-center mt-1">
					<div className={`w-2 h-2 rounded-full ${
						hasLiveMatch ? 'bg-red-500' : 
						hasCompletedMatch ? 'bg-green-500' : 
						'bg-blue-500'
					}`}></div>
				</div>
			);
	...
	return (
		<div className="relative flex size-full min-h-screen flex-col bg-slate-50 overflow-x-hidden font-['Manrope','Noto_Sans',sans-serif]">
						...
						<div className="flex flex-wrap justify-between gap-3 p-4">
							<div className="flex min-w-72 flex-col gap-3">
								...
								<p className="text-[#60758a] text-sm font-normal leading-normal font-['Manrope',Helvetica]">
									View and manage matches across all age groups and competitions
								</p>
							</div>
						</div>
					...
					{selectedMatch && (
						<>
							...
							<div className="space-y-6">
								...
								{selectedMatch.status === 'completed' && selectedMatch.homeScore !== undefined && selectedMatch.awayScore !== undefined && (
									<div className="text-center p-4 bg-green-50 rounded-lg">
										<div className="text-3xl font-bold text-[#111418] font-['Manrope',Helvetica]">
											{selectedMatch.homeScore} - {selectedMatch.awayScore}
										</div>
										<p className="text-sm text-[#60758a] font-['Manrope',Helvetica] mt-1">Final Score</p>
									</div>
								)}
								...
								<div className="grid grid-cols-2 gap-4">
									<div className="space-y-3">
										<div className="flex items-center gap-2 text-sm">
											...
											<span className="font-medium font-['Manrope',Helvetica]">Kick-off:</span>
											<span className="text-[#60758a] font-['Manrope',Helvetica]">{selectedMatch.time}</span>
										</div>
										<div className="flex items-center gap-2 text-sm">
											<MapPin className="w-4 h-4 text-[#60758a]" />
											<span className="font-medium font-['Manrope',Helvetica]">Venue:</span>
											<span className="text-[#60758a] font-['Manrope',Helvetica]">{selectedMatch.venue}</span>
										</div>
										<div className="flex items-center gap-2 text-sm">
											<Users className="w-4 h-4 text-[#60758a]" />
											<span className="font-medium font-['Manrope',Helvetica]">Age Group:</span>
											<span className="text-[#60758a] font-['Manrope',Helvetica]">{selectedMatch.ageGroup}</span>
										</div>
									</div>
									<div className="space-y-3">
										<div className="text-sm">
											<span className="font-medium font-['Manrope',Helvetica]">Competition:</span>
											<span className="text-[#60758a] ml-2 font-['Manrope',Helvetica]">{selectedMatch.competition}</span>
										</div>
										<div className="text-sm">
											<span className="font-medium font-['Manrope',Helvetica]">Referee:</span>
											<span className="text-[#60758a] ml-2 font-['Manrope',Helvetica]">{selectedMatch.referee}</span>
										</div>
										<div className="text-sm">
											<span className="font-medium font-['Manrope',Helvetica]">Status:</span>
											<Badge 
												variant="secondary" 
												className={`ml-2 text-xs ${getStatusColor(selectedMatch.status)}`}
											>
								...
								{(selectedMatch.weather || selectedMatch.attendance) && (
									<div className="grid grid-cols-2 gap-4 p-3 bg-gray-50 rounded-lg">
										{selectedMatch.weather && (
											<div className="text-sm">
												<span className="font-medium font-['Manrope',Helvetica]">Weather:</span>
												<span className="text-[#60758a] ml-2 font-['Manrope',Helvetica]">{selectedMatch.weather}</span>
											</div>
										)}
										{selectedMatch.attendance && (
											<div className="text-sm">
												<span className="font-medium font-['Manrope',Helvetica]">Attendance:</span>
												<span className="text-[#60758a] ml-2 font-['Manrope',Helvetica]">{selectedMatch.attendance}</span>
											</div>
										)}
									</div>
								)}
								...
								{/* Description */}
								<div>
									<h4 className="font-semibold text-[#111418] mb-2 font-['Manrope',Helvetica]">Match Preview</h4>
								...
								{/* Team Sheet */}
								<div>
									<h4 className="font-semibold text-[#111418] mb-2 font-['Manrope',Helvetica]">Team Information</h4>
									<div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-3 bg-blue-50 rounded-lg">
										<div className="text-sm">
											<span className="font-medium font-['Manrope',Helvetica]">Formation:</span>
											<span className="text-[#60758a] ml-2 font-['Manrope',Helvetica]">{selectedMatch.teamSheet.formation}</span>
										</div>
										<div className="text-sm">
											<span className="font-medium font-['Manrope',Helvetica]">Captain:</span>
											<span className="text-[#60758a] ml-2 font-['Manrope',Helvetica]">{selectedMatch.teamSheet.captain}</span>
										</div>
										<div className="text-sm">
											<span className="font-medium font-['Manrope',Helvetica]">Key Players:</span>
											<div className="mt-1">
												{selectedMatch.teamSheet.keyPlayers.map((player, index) => (
													<Badge key={index} variant="outline" className="text-xs mr-1 mb-1 font-['Manrope',Helvetica]">
														{player}
													</Badge>
												))}
											</div>
								...
								{/* Match Objectives */}
								<div>
									<h4 className="font-semibold text-[#111418] mb-2 font-['Manrope',Helvetica]">Match Objectives</h4>
									<ul className="space-y-1">
										{selectedMatch.objectives.map((objective, index) => (
			...
			<style jsx>{`
				...
				.match-calendar .react-calendar__navigation button:enabled:hover,
				.match-calendar .react-calendar__navigation button:enabled:focus {
					background-color: #f3f4f6;
				}
			`}</style>
		</div>
